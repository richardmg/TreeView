...
/*!
    ...
    tree nodes that are \i visible in the view are flattened into a list that can be
...
/*!
    ...
    \note Using \l currentItem is \i almost the same as calling
    \l itemAtModelIndex(currentModelIndex). The difference is that the latter
    ...
    if \l currentIndex.valid is \c false, this property will be \c null.

    \sa itemAtCell(), currentIndex, currentModelIndex()
*/
...
/*!
    ...
    \list
    \li TreeView.List - the tree view acts like as a list view. This means that
    ...
    \li TreeView.Table - the tree view acts like a table view. This means that
...
/*!
    ...
    appearance of the \l delegate without the need to create a custom one.
    The hints that can be set is the following:

    \list
    \li styleHints.indicator - the color of the collapsed/expanded icon
    \li styleHints.overlay - the color of the overlay used for \currentIndex
    \li styleHints.foregroundOdd - the foreground color of odd rows
    \li styleHints.backgroundOdd - the background color of odd rows
    \li styleHints.foregroundEven - the foreground color of even rows
    \li styleHints.backgroundEven - the background color of even rows
    \li styleHints.foregroundCurrent - the foreground color of the current item (or
        the whole row, if \l selectionMode is \c TreeView.List)
    \li styleHints.backgroundCurrent - the background color of the current item (or
        the whole row, if \l selectionMode is \c TreeView.List)
    \li styleHints.indent - the horizontal space between a parent and a child node
    \li styleHints.columnPadding - the padding between a cell and its contents
    \li styleHints.font - the font used by text items in the delegate
    \endlist
...
/*!
    ...
    Returns if the given \row in the view is shown as
    expanded.
...
/*!
    ...
    Returns if the given \row in the view has
    siblings.
...
/*!
    ...
    given \row in the view.

    \note this function will not affect the model, only
    the visual representation in the view.

    \sa viewIndex, collapse(), expandModelIndex(), collapseModelIndex(), isCollapsed()
*/
...
/*!
    ...
    \sa viewIndex, expand(), expandModelIndex(), collapseModelIndex(), isCollapsed()
*/

/*!
    \qmlmethod QtQuick::TreeView::toggleExpanded(row)

    Toggles if the tree node at the given \l row
...
/*!
    ...
    Returns if the given \modelIndex is shown as expanded
...
/*!
    ...
    Collapses the tree node at the
    given \modelIndex. This is a convenience for doing:

    \code
    collapse(mapFromModel(modelIndex).row)
    \endcode

    \sa viewIndex, collapse(), expand(), expandModelIndex(), isCollapsed()
*/
...
/*!
    ...
    \sa viewIndex, collapse(), expand(), collapseModelIndex(), isCollapsed()
*/

/*!
    \qmlmethod QtQuick::TreeView::toggleModelIndexExpanded(modelIndex)

    Toggles if the tree node at the given \l modelIndex
    should be expanded. This is a convenience for doing:
...
/*!
    \qmlmethod int QtQuick::TreeView::columnAtPos(x, y, includeSpacing)

    Returns the column under the position \l x, \l y
    in view coordinates.

    If \l includeSpacing is set to \l false, and the position is on top
    of the spacing between the columns, the return value will be -1.
    Otherwise, if the position is on top of the spacing and \l includeSpacing
    is set to \c true, the column closest to the position will be returned.

    \sa columnSpacing
*/

/*!
    \qmlmethod int QtQuick::TreeView::rowAtPos(x, y, includeSpacing)

    Returns the row under the position \l x, \l y
    in view coordinates.

    If \l includeSpacing is set to \l false, and the position is on top
    of the spacing between the rows, the return value will be -1.
    Otherwise, if the position is on top of the spacing and \l includeSpacing
    is set to \c true, the row closest to the position will be returned.
...
/*!
    ...
    Returns the item inside the given table cell. If the cell
    is not visible in the view, \c null will be returned.
...
/*!
    ...
    Returns the item at the given view index. If \c viewIndex.valid
    is \false, \c null will be returned.
...
/*!
    ...
    represents the model index is not visible in the view, \c null
...
/*!
    ...
    a model index to use with the model, refer to the \l model API.
*/
...
/*!
    ...
    corresponding model index in the \l model.
...
/*!
    ...
    Maps an index pointing to an item in the \l model to the
